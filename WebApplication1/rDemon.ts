 ...
 */
enum GameState {
  Waiting,      // waiting for players to join and game to start
  Playing,      // started and in progress 
  Scoring,      // game over sorting cards and calculating round score
  ShowingScores,// Finished above, showing overall scores, ready to start next round
  GameOver      // As above, but we have an overall winner, ready to start new game
}
...
class RacingDemon {
  ...
  dragCardRank = 0;         // if it was a king ...
  ...
  roundScores = [0, 0, 0, 0];
  allScores: number[][] = [];     // allScores[roundI][playerI] = score of playerI in roundI 
  ...
  gameStateSet(newState: GameState) {    
  ...
  unloadFunc(event: BeforeUnloadEvent) {    
  ...
  requestDrag(pileI: number, cardI: number): number {
    ...
    if (pileI < wastePileI || pileI > rWorkPileI) { return 0 };
  ...
  requestDrop(pileI: number, cardI: number, x: number, y: number): number {
  ...
  actionsAfterOut(outPlayer: string) {
  ...
  ReadyToSort(lcpPileI: number, lcpX: number, lcpY: number) {
  ...
  playerDeparted(deserterName: string) {
    ...
    let flashes = 10;
    const desPlayerI = this.players.findIndex((name: string): boolean => {return (name == deserterName) })
