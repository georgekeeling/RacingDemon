...
class Card {
    constructor(pile, playerI, cards52I, xPos, yPos, faceUp, angle) {
        this.pile = pile;
    ...
    isBlack() {
        if (this.cards52I < 13 || this.cards52I >= 39)
            return true;
        return false;
    }
}
class Pile {
    ...
    addCardP(playerI, cardI, x, y, faceUp, angle) {
    ...
    receive(pileI, pileX, pileY, bCardsIn) {
        ...
        for (let cardI = 0; cardI < bCardsIn.length; cardI++) {
            let bCard = new Bcard(bCardsIn[cardI].playerI, bCardsIn[cardI].cards52I, bCardsIn[cardI].x, bCardsIn[cardI].y, bCardsIn[cardI].faceUp, bCardsIn[cardI].angle);
...
class Bcard {
    constructor(playerI, cards52I, x, y, faceUp, angle) {
        this.playerI = playerI;
        this.cards52I = cards52I;
        this.x = x;
        this.y = y;
...
class Table {
    constructor() {
        ...
        this.SVGheight = 540;
        this.commonArea = new Area(); // common area, different for tablet / smartphone
        this.width = 1000;
        this.height = 600;
        this.aspectRatio = 0; // aspect ratio of canvas (= table). 1 on tablet, width/height on smart phone.
        ...
        this.lockLevel = 0;
        this.lockTime = new Date(); // diagnostic, to see if lock gets permanent for bugs 10/7/23 & 25/6/23 & others YES
        this.lockMaxTime = 60000; // diagnostic ditto. was 1 minute (60000 ms). 4 player game sort after endGame(15) took 38 seconds
        this.prevElapsedMs = -10000; // diagnostic ditto
        this.piles = []; // piles of cards on the table. Obviously!
        this.startAllowedFlasherID = 0; // setInterval returns "a numeric, non-zero value"
        this.tip = ""; // current tip drawn on table border above players home
        this.circles = []; // centres of up to two circles in common area
        this.roundCounter = 0;
        this.gameCounter = 1;
    ...
    writeText(text, x, y, ifOverlaps) {
        ...
        if (typeof (ifOverlaps) != 'undefined') {
            ...
            if (!textArea.overlaps(ifOverlaps)) {
                return height;
            }
        }
    ...
    writeCentralBigRandomFont(written) {
        ...
        this.ctx.textAlign = "center";
        this.ctx.clearRect(this.commonArea.left + 2, topCL, this.commonArea.right - this.commonArea.left - 4, heightCL * 1.2);
        // draw centred text. X coord is centre of text. Y coord (parameter 3) is where BOTTOM of text is!!!
    ...
    showCards(area) {
        ...
        this.ctx.strokeRect(this.commonArea.left, this.commonArea.top, this.commonArea.right - this.commonArea.left, this.commonArea.bottom - this.commonArea.top);
        if (this.tip != "") {
            this.writeTip2();
            this.drawCircles();
        }
        function showPile(aPile) {
            ...
            for (let cardI = 0; cardI <= aPile.cards.length - 1; cardI++) {
                ...
                if (area.overlaps(thisCard.area) || hadFirstVisible) {
                    ...
                    if (!thisCard.faceUp) {
                        CardImg = pack.cardBacks[thisCard.playerI];
                    }
                    ;
                    if (thisCard.angle != 0) {
    ...
    addCardT(pile, playerI, cardIx, x, y, faceUp, angle) {
...
class Pack {
    constructor() {
        ...
        this.shuffled = [];
        this.cardBacks = [document.getElementById("cBack0"), document.getElementById("cBack1"),
